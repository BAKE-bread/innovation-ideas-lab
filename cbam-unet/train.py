...
def main():
    ...
    if LOAD_MODEL:
        try:
            checkpoint = torch.load("best_cbam_unet_pets.pth.tar", weights_only=True)
    ...
    PATIENCE = 10  # 允许 10 个 epoch 没有提升
    patience_counter = 0
    best_val_dice = 0.0

    # 把 NUM_EPOCHS 设得很大，让早停来决定何时停止
    NUM_EPOCHS = 200 # 比如 200
    print(f"--- 开始训练，使用早停策略 (Patience={PATIENCE}) ---")
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        current_val_dice = check_accuracy(val_loader, model, device=DEVICE)

        if current_val_dice > best_val_dice:
            # 新高
            print(f"Epoch {epoch+1}: 验证Dice分数提升 ({best_val_dice:.4f} -> {current_val_dice:.4f}). 保存模型...")
            ...
            torch.save(checkpoint, "best_cbam_unet_pets.pth.tar") 

        else:
            # 没有提升
            patience_counter += 1
            print(f"Epoch {epoch+1}: 验证Dice分数未提升. 最佳: {best_val_dice:.4f}. (Patience {patience_counter}/{PATIENCE})")

        # 检查是否触发早停
        if patience_counter >= PATIENCE:
            print(f"--- 早停触发！连续 {PATIENCE} 轮未提升. 训练结束. ---")
            print(f"最佳Dice分数为: {best_val_dice:.4f}")
            break # 跳出 epoch 循环
    ...
    #     "optimizer": optimizer.state_dict(),
    # }
    # torch.save(checkpoint, "cbam_unet_pets_es.pth.tar")
